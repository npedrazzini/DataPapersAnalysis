...
df_dimens = pd.read_csv('./zenodo_dimensions_all_humss/dimensions_humss_research.csv')

#Drop duplicates
df_dimens = df_dimens.drop_duplicates('Publication ID', keep='last')

df_linked1 = pd.read_csv("/Users/npedrazzini/Desktop/johd_crawler/outputs/final_outputs/research_papers/{}-final-research_papers-rdj.csv".format(dateoflastdf))
df_linked2 = pd.read_csv("/Users/npedrazzini/Desktop/johd_crawler/outputs/final_outputs/research_papers/{}-final-research_papers-johd.csv".format(dateoflastdf))
...
datedf_linked = datetime.strptime('2022-06-05', date_format)
...
with open('./outputs/analysis_outputs/{}-means-citations-analysis-B.csv'.format(dateoflastdf), 'w') as outcsv:
    ...
    for _ in itertools.repeat(None, 10):
        df_dimens = df_dimens.dropna(subset=['Times cited'])
        df_linked2 = df_linked2.dropna(subset=['Times cited'])

        y = random.sample(sorted(list(df_dimens['Times cited'])), 5000)
        ...
        outcsv.writerow(['all_zenodo_downloads','datasets_withDP_downloads',all_research_citations_mean,research_withDP_citations_mean,welchs_stats_citations,welchs_p_citations,mannwhit_stats_citations,mannwhit_p_citations])
...
df_dimens2 = df_dimens.dropna(subset=['Altmetric'])
df_linked2 = df_linked.dropna(subset=['Altmetric'])

# Remove outliers
q1, q3= np.percentile(sorted(df_dimens2['Altmetric']),[25,75])
iqr = q3 - q1
lower_bound_citations = q1 -(1.5 * iqr) 
upper_bound_citations = q3 +(1.5 * iqr) 
outlier_max_citations = upper_bound_citations         # Set the outlier threshold (max)
...
df_dimens2  = df_dimens2.loc[(df_dimens2['Altmetric'] <= outlier_max_citations) & (df_dimens2['Altmetric'] >= outlier_min_citations)] 

# Remove outliers
q1, q3= np.percentile(sorted(df_linked2['Altmetric']),[25,75])
iqr = q3 - q1
lower_bound_citations = q1 -(1.5 * iqr) 
upper_bound_citations = q3 +(1.5 * iqr) 
outlier_max_citations = upper_bound_citations         # Set the outlier threshold (max)
...
df_linked2  = df_linked2.loc[(df_linked2['Altmetric'] <= outlier_max_citations) & (df_linked2['Altmetric'] >= outlier_min_citations)] 
...
with open('./outputs/analysis_outputs/{}-means-altmetric-analysis-B.csv'.format(dateoflastdf), 'w') as outcsv:
    ...
    outcsv.writerow(['variable1','variable2','mean_variable1','mean_variable2','welchs_stats','welchs_pvalue','mannwhit_stats','mannwhit_pvalue'])
    for _ in itertools.repeat(None, 10):
        df_dimens = df_dimens.dropna(subset=['Altmetric'])
        df_linked2 = df_linked2.dropna(subset=['Altmetric'])

        y = random.sample(sorted(list(df_dimens['Altmetric'])), 5000)
        
        all_research_altmetric_mean = mean(y)
        research_withDP_altmetric_mean = mean(df_linked2['Altmetric'])

        welchs_stats_altmetric, welchs_p_altmetric = stats.ttest_ind(df_linked2['Altmetric'], y, equal_var = False)
        mannwhit_stats_altmetric, mannwhit_p_altmetric = stats.mannwhitneyu(df_linked2['Altmetric'], y)
        
        outcsv.writerow(['all_zenodo_downloads','datasets_withDP_downloads',all_research_altmetric_mean,research_withDP_altmetric_mean,welchs_stats_altmetric,welchs_p_altmetric,mannwhit_stats_altmetric,mannwhit_p_altmetric])

# Export df for R boxplots
x = random.sample(sorted(list(df_dimens2['Altmetric'])), 5000)
...
array1 = [df_linked2['Altmetric'],x]
plt.boxplot(array1)
# plt.ylim(bottom_value,top_value)
plt.title('Difference in means: altmetric all Hum. res. papers/altmetric. res. papers with data paper')
plt.show()

array1 = [df_linked2['Times cited'],y]
plt.boxplot(array1)
# plt.ylim(bottom_value,top_value)
plt.title('Difference in means: cit. all Hum. res. papers/cit. res. papers with data paper')
plt.show()
